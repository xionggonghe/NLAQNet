...
def quaternion_pool(X, kernel_size,stride):
    X = X.float().to(device)
    k_h, k_w = kernel_size
    # print("x: ", X.shape)
    Qchannel = X.shape[1] // 4
    Y = torch.zeros(X.shape[0], X.shape[1], (X.shape[2] - k_h + 1) // stride + 1,
                    (X.shape[3] - k_w + 1) // stride + 1).to(device)
    r = get_r(X).to(device)
    i = get_i(X).to(device)
    j = get_j(X).to(device)
    k = get_k(X).to(device)
    norm = torch.sqrt(r * r + i * i + j * j + k * k + 0.00001).to(device)
    # indexList = []
    for h in range(0, X.shape[2], stride):
        yh = h // stride
        for w in range(0, X.shape[3], stride):
            yw = w // stride
            # 计算指导矩阵
            ker = norm[:, :, h: h + k_h, w: w + k_w].reshape(Y.shape[0], Qchannel, -1, k_h * k_w).clone().to(device)
            # k2 = ker.numpy()
            maxindex = torch.argmax(ker, dim=3).to(device)
            ker = F.one_hot(maxindex[:, :, :]).to(device)
            # indexList.append(ker)
            if (ker.shape[3] == 1):
                dimzero = torch.zeros([Y.shape[0], Qchannel, 1, 1]).to(device)
                ker = torch.cat((ker, dimzero), dim=3)
            if (ker.shape[3] == 2):
                dimzero = torch.zeros([Y.shape[0], Qchannel, 1, 1]).to(device)
                ker = torch.cat((ker, dimzero), dim=3)
            if (ker.shape[3] == 3):
                dimzero = torch.zeros([Y.shape[0], Qchannel, 1, 1]).to(device)
                ker = torch.cat((ker, dimzero), dim=3)
            # ker = ker.reshape(Y.shape[0], Qchannel, k_h, k_w)
            # k1 = ker.numpy()
            # index_h = torch.div(maxindex, k_w, rounding_mode='trunc')
            # index_w = torch.div(maxindex, k_w, rounding_mode='trunc')
            # index_h, index_w = maxindex//k_w, maxindex % k_w

            # t = ( * ) #.sum(axis=3).sum(axis=2)
            # t1 = t.numpy()
            Y[:, :Qchannel, yh, yw] = torch.sum((X[:, :Qchannel, h: h + k_h, w: w + k_w].reshape(Y.shape[0], Qchannel, -1, k_h * k_w) * ker), axis=(2, 3))
            Y[:, Qchannel:Qchannel * 2, yh, yw] = torch.sum((X[:, Qchannel:Qchannel * 2, h: h + k_h, w: w + k_w].reshape(Y.shape[0], Qchannel, -1, k_h * k_w) * ker), axis=(2, 3))
            Y[:, Qchannel * 2:Qchannel * 3, yh, yw] = torch.sum((X[:, Qchannel * 2:Qchannel * 3, h: h + k_h, w: w + k_w].reshape(Y.shape[0], Qchannel, -1, k_h * k_w) * ker), axis=(2, 3))
            Y[:, Qchannel * 3:Qchannel * 4, yh, yw] = torch.sum((X[:, Qchannel * 3:Qchannel * 4, h: h + k_h, w: w + k_w].reshape(Y.shape[0], Qchannel, -1, k_h * k_w) * ker), axis=(2, 3))
    return Y, ker


def quaternion_poolBackward(X, kernel, grad):
    # i = 0
    # print("pool Backward: ")
    # print(grad.shape)
    outgrad = torch.zeros_like(X)
    Qchannel = X.shape[1] // 4
    kersize = int(pow(kernel.shape[3], 0.5))
    k_h = kersize
    k_w = kersize
    stride = int((X.shape[3] - kersize + 1) // grad.shape[3])+1
    r = get_r(X).to(device)
    i = get_i(X).to(device)
    j = get_j(X).to(device)
    k = get_k(X).to(device)
    ...
    for h in range(0, X.shape[2], stride):
        ...
        for w in range(0, X.shape[3], stride):
            ...
            yw = w // stride
            # 计算指导矩阵
            ker = norm[:, :, h: h + k_h, w: w + k_w].reshape(X.shape[0], Qchannel, -1, k_h * k_w).clone().to(device)
            maxindex = torch.argmax(ker, dim=3).to(device)
            ker = F.one_hot(maxindex[:, :, :]).to(device)
            if (ker.shape[3] == 1):
                dimzero = torch.zeros([X.shape[0], Qchannel, 1, 1]).to(device)
                ker = torch.cat((ker, dimzero), dim=3)
            if (ker.shape[3] == 2):
                dimzero = torch.zeros([X.shape[0], Qchannel, 1, 1]).to(device)
                ker = torch.cat((ker, dimzero), dim=3)
            ...
            outgrad[:, :Qchannel,  h: h + k_h, w: w + k_w] += \
                (ker.reshape(ker.shape[0], ker.shape[1], kersize, kersize)) * (grad[:, :grad.shape[1]//4, yh, yw].reshape(grad.shape[0], grad.shape[1]//4, 1, 1))
            outgrad[:, Qchannel:Qchannel * 2, h: h + k_h, w: w + k_w] += \
                (ker.reshape(ker.shape[0], ker.shape[1], kersize, kersize)) * (grad[:, grad.shape[1]//4:grad.shape[1]//2, yh, yw].reshape(grad.shape[0], grad.shape[1]//4, 1, 1))
            outgrad[:, Qchannel * 2:Qchannel * 3, h: h + k_h, w: w + k_w] += \
                (ker.reshape(ker.shape[0], ker.shape[1], kersize, kersize)) * grad[:, grad.shape[1]//2:grad.shape[1]//4*3, yh, yw].reshape(grad.shape[0], grad.shape[1]//4, 1, 1)
            outgrad[:, Qchannel * 3:Qchannel * 4, h: h + k_h, w: w + k_w] += \
                (ker.reshape(ker.shape[0], ker.shape[1], kersize, kersize)) * grad[:, grad.shape[1]//4*3:grad.shape[1], yh, yw].reshape(grad.shape[0], grad.shape[1]//4, 1, 1)
    return outgrad


def quaternion_sigmoid(input):
    ...
    # 计算exp(inp)
    exp    = torch.exp(r)
    r      = torch.cos(norm_v)*exp
    i      = (i / norm_v) * torch.sin(norm_v)*exp
    j      = (j / norm_v) * torch.sin(norm_v)*exp
    k      = (k / norm_v) * torch.sin(norm_v)*exp

    # 由于四元数没有显式除法，使用取逆的方式计算除法
    # 1/Q = 1/(a+bi+cj+dk) = (a-bi-cj-dk)/(a**2 + b**2 + c**2 + d**2)
    # 计算(1 + exp(inp))的逆
    r = r + 1
    norm_out2 = r*r+i*i+j*j+k*k
    i = -1 * i
    j = -1 * j
    k = -1 * k
...
def quaternion_sigmoidBackward(input):
    r = get_r(input)
    i = get_i(input)
    j = get_j(input)
    k = get_k(input)
...
def quaternion_relu(input):
    ...
    out_r = torch.where(norm1 < 0., torch.zeros_like(r), r)
    out_i = torch.where(norm1 < 0., torch.zeros_like(i), i)
    out_j = torch.where(norm1 < 0., torch.zeros_like(j), j)
    out_k = torch.where(norm1 < 0., torch.zeros_like(k), k)

    return torch.cat([out_r, out_i, out_j, out_k], dim=1)

def quaternion_reluBackward(input):
    ...
    out_r = torch.where(norm1 < 0., torch.zeros_like(r), torch.ones_like(r))
    out_i = torch.where(norm1 < 0., torch.zeros_like(i), torch.ones_like(i))
    out_j = torch.where(norm1 < 0., torch.zeros_like(j), torch.ones_like(j))
    out_k = torch.where(norm1 < 0., torch.zeros_like(k), torch.ones_like(k))
...
def quaternion_init(in_features, out_features, rng, kernel_size=None, criterion='glorot'):
    ...
    if kernel_size is not None:
        receptive_field = np.prod(kernel_size)  #计算乘积大小
        fan_in          = in_features  * receptive_field
        fan_out         = out_features * receptive_field
    else:
        fan_in          = in_features
