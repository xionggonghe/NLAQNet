...
from   model.Qmodule.quaternion_ops          import *
import math
import sys
...
class QuaternionConv(Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride,
        ...
        # print("test_w_shape")
        # print(self.w_shape)
        w=torch.Tensor(*self.w_shape)
        self.r_weight  = Parameter(nn.init.xavier_normal_(w.clone(), gain=1))
        self.i_weight  = Parameter(nn.init.xavier_normal_(w.clone(), gain=1))
        self.j_weight  = Parameter(nn.init.xavier_normal_(w.clone(), gain=1))
        self.k_weight  = Parameter(nn.init.xavier_normal_(w.clone(), gain=1))
        if self.scale:
            self.scale_param  = Parameter(nn.init.xavier_normal_(w, gain=1))
        else:
