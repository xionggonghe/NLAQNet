...
from model.ViTmodule.transformer import TransformerEncoder, LinearAttnFFN
from model.ViTmodule.base_module import BaseModule
from model.ViTmisc.profiler import module_profile
from model.ViTlayers.conv_layer import ConvLayer
from model.ViTlayers.normalization_layers import get_normalization_layer
from model.Qconv import QuaternionConv as QConv


class QDilatedInception(BaseModule):
    def __init__(self, in_channel, out_channel):
        super(QDilatedInception, self).__init__()

        self.conv1 = QConv(in_channels=in_channel, out_channels=out_channel, kernel_size=3,
                           stride=1, padding=1, bias=False, groups=1, dilatation=1)
        self.conv2 = QConv(in_channels=in_channel, out_channels=out_channel, kernel_size=3,
                           stride=1, padding=3, bias=False, groups=1, dilatation=3)
        self.conv3 = QConv(in_channels=in_channel, out_channels=out_channel, kernel_size=3,
                           stride=1, padding=5, bias=False, groups=1, dilatation=5)
        self.conv4 = QConv(in_channels=in_channel, out_channels=out_channel, kernel_size=1,
                           stride=1, padding=0, bias=False, groups=1, dilatation=1)
...
class DilatedInception(BaseModule):
    def __init__(self, in_channel, out_channel):
        super(DilatedInception, self).__init__()
        self.conv1 = ConvLayer(opts=None, in_channels=in_channel, out_channels=out_channel, kernel_size=3, stride=1,
                               use_norm=True, use_act=True, dilation=1, groups=in_channel,)
        self.conv2 = ConvLayer(opts=None, in_channels=in_channel, out_channels=out_channel, kernel_size=3, stride=1,
                               use_norm=True, use_act=True, dilation=3, groups=in_channel, )
        self.conv3 = ConvLayer(opts=None, in_channels=in_channel, out_channels=out_channel, kernel_size=3, stride=1,
                               use_norm=True, use_act=True, dilation=5, groups=in_channel, )
        self.conv4 = ConvLayer(opts=None, in_channels=in_channel, out_channels=out_channel, kernel_size=1, stride=1,
                               use_norm=True, use_act=True, dilation=1, groups=in_channel, )
...
class QMobViTBlock(BaseModule):
    """
    This class defines the `MobViTBlock block <>`_
    ...
    def __init__(
        ...
        conv_3x3_in = QDilatedInception(in_channels, cnn_out_dim)
        conv_1x1_in = QConv(in_channels=cnn_out_dim*4, out_channels=cnn_out_dim, kernel_size=1, stride=1, padding=0, bias=False, groups=1)

        super(QMobViTBlock, self).__init__()
        ...
        self.conv_proj = QConv(in_channels=cnn_out_dim, out_channels=in_channels, kernel_size=1,
              stride=1, padding=0, bias=False, groups=1, dilatation=1)
        ...
        self.enable_coreml_compatible_fn = getattr(
            None, "common.enable_coreml_compatible_module", False
        )
...
class MobileViTBlockv3(BaseModule):
    ...
    def __init__(
        ...
        conv_3x3_in = DilatedInception(in_channels, cnn_out_dim)
        conv_1x1_in = ConvLayer(
            opts=opts,
            in_channels=cnn_out_dim * 4,
